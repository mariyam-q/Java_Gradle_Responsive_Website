pipeline {
    ...
    stages {
        ...
        stage('Push Docker Image to Nexus') {
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'nexus', usernameVariable: 'admin', passwordVariable: 'mariyamV@123')]) {
                        sh '''
                        sudo docker tag mariyamq/gradle1:latest 20.70.128.111:8083/gradle1:latest
                        sudo docker login -u admin -p mariyamV@123 20.70.128.111:8083
                        sudo docker push 20.70.128.111:8083/gradle1:latest
                        '''
                    }
                }
            }
        ...
        stage('Deploy to Container') {
            steps {
                script {
                    sh 'sudo docker run -d --name g1 -p 8082:8080 20.70.128.111:8083/gradle1:latest'
                }
            }
        }
    }
